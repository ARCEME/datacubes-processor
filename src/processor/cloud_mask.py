...
# def process_directory(input_dir, output_dir):
#     """Process all Zarr files in the input directory."""
#     input_dir = Path(input_dir)
#     output_dir = Path(output_dir)
#     output_dir.mkdir(parents=True, exist_ok=True)
...
# /home/eouser/miniconda3/envs/cubecloud/bin/python
...
#     # Check for duplicate times
#     time_series = ds['time_s2'].to_series()
#     if time_series.duplicated().any():
#         print(f"Warning: Duplicate time entries found in {zarr_input_path}")
#         duplicates = time_series[time_series.duplicated(keep=False)]
#         print(f"Duplicate timestamps: {duplicates.index.tolist()}")
...
#     ds_out = ds.copy(deep=True)
#     # Process each unique date
#     for time_s2 in ds.time_s2.values: #time_sentinel_2_l2a
#         # Check for Sentinel-2 data availability
#         s2_available = any(band in ds.data_vars and not ds[band].sel(time_s2=time_s2).isnull().all()
#                           for band in S2_RAW_BANDS)
        
#         if not s2_available:
#             print(f"No valid Sentinel-2 bands for time {time_s2} in {zarr_input_path}")
#             continue
...
#         available_bands = []
#         for band in S2_RAW_BANDS:
#             if band in ds.data_vars and not ds[band].sel(time_s2=time_s2).isnull().all():
#                 band_data = ds[band].sel(time_s2=time_s2).values
#                 if band_data.ndim != 2:
#                     print(f"Error: {band} at time {time_s2} has unexpected shape {band_data.shape}")
#                     continue
#                 band_data = np.nan_to_num(band_data, nan=0.0).astype(np.float32)
#                 band_data = (band_data - 1000) / 10000  # Normalize
#                 s2_data.append(band_data)
...
#         if not s2_data:
#             print(f"No valid bands for time {time_s2} in {zarr_input_path}")
#             continue
...
#         print(f"Time {time_s2}: {len(available_bands)} bands, shape {s2_array.shape}")
...
#             if 'cloud_mask' not in ds_out:
#                 ds_out['cloud_mask'] = xr.DataArray(
#                     np.full((ds.sizes['time_s2'], height, width), np.nan, dtype=np.float32),
#                     dims=['time_s2', 'y', 'x'],
#                     coords={'time_s2': ds.time_s2, 'y': ds.y, 'x': ds.x}
#                 )
            
#             # Store cloud mask
#             ds_out['cloud_mask'].loc[dict(time_s2=time_s2)] = cloud_mask
            
#         except Exception as e:
#             print(f"Error processing time {time_s2} in {zarr_input_path}: {str(e)}")
#             continue
...
# # Example usage
# if __name__ == "__main__":
#     input_directory = '/ARCEMECUBES/STAGING_CUBES/test_2years_clouds/'
#     output_directory = '/ARCEMECUBES/PRODUCTION_CUBES/CLOUDFERRO/S2L2A_CLOUDMASK/'
#     process_directory(input_directory, output_directory)

...
from utils import process_bounding_box, search_stac_items, save_cube_with_retries, get_location_data, build_cubedataset_from_items, get_processed_locations, central_pixel_bbox, compute_distance_to_center, harmonize_to_pixels, get_stac_client
...
dc_locations_table_path =  '/home/eouser/datacubes/arceme-datacubes/arceme-datacubes/datasets/dhp_global_subselection_new_melanie.csv'# '/home/eouser/datacubes/arceme-datacubes/arceme-datacubes/datasets/1_max_precipitation_grid_cells.csv' # '/home/eouser/datacubes/arceme-datacubes/arceme-datacubes/datasets/dhp_global_subselection_mod_MK.csv'
table = pd.read_csv(dc_locations_table_path)
nrow = table.shape[0]
base_output_dir = '/ARCEMECUBES/NEW-CUBES-MELANIE/S2L2A/'  #'/ARCEMECUBES/PRODUCTION_CUBES/CLOUDFERRO/S2L2A/' # '/ARCEMECUBES/MELANIE_DC/S2L2A/' #'/ARCEMECUBES/PRODUCTION_CUBES/CLOUDFERRO/S2L2A/'
log_file = os.path.join(base_output_dir, "processing_times.txt")
max_retries = 3
# log_path = os.path.join(base_output_dir, "gdal_http.log")
# os.environ["CPL_LOG"] = log_path
# os.environ["CPL_DEBUG"] = "ON" 
# os.environ["CPL_CURL_VERBOSE"] = "YES" 
processed_locations = get_processed_locations(base_output_dir)
print(f"Already processed: {len(processed_locations)} locations")
...
for i in range(nrow):
    ...
    if location in processed_locations:
        print(f"[SKIP] {location} already processed")
        continue
    ...
    if data_collections == 'sentinel-2-l2a':
        polygon = box(bbox_latlon[0], bbox_latlon[1], bbox_latlon[2], bbox_latlon[3])
        geojson_polygon = mapping(polygon)
        my_polygon = shape(geojson_polygon)

        items = [
            item for item in items
            if shape(item.to_dict()["geometry"]).contains(my_polygon)
        ]
